	...
	*[Symbol.iterator](allowInfinity = false) {
	...
	slice(offset = 0, length = Infinity) {
		...
		isInfinity(this._length) && check(nOffset, [
			[ x => x < 0,	`* FArrayT~slice: offset.need.non-negative.number` 	]]);
		...
		const realStart    = this._start - (this._reversed ? this._length : 0           ),
			  ...
			  rangeLimit   = limit(realStart, realEnd);
		const tmpOffset    = nOffset,
			  tmpStart     = realStart + tmpOffset,
			  tmpEnd       = tmpStart + nLength;
		const wantStart    = nLength >= 0 ? tmpStart : tmpEnd,
			  wantEnd      = nLength >= 0 ? tmpEnd   : tmpStart,
			  wantLength   = nLength >= 0 ? nLength  : -nLength;
		const adjustStart  = rangeLimit(wantStart),
			  adjustEnd    = rangeLimit(wantEnd),
			  adjustLength = adjustEnd - adjustStart;
		const newReversed  = nLength < 0,
			  newStart     = adjustStart + (newReversed ? adjustLength : 0),
			  newLength    = adjustLength;
	...
	roundSlice(offset = 0, length = Infinity) {
		...
		isInfinity(this._length) && check(nOffset, [
			[ x => x < 0,	`* FArrayT~roundSlice: offset.need.non-negative.number` ]]);
		const nLength      = check(Math.trunc(length), [
			[ isNaN,		`* FArrayT~roundSlice: length.need.number`				]]);
		const realStart    = this._start - (this._reversed ? this._length : 0           ),
			  realEnd	   = this._start + (this._reversed ? 0            : this._length),
			  rangeLimit   = limit(realStart, realEnd);
		const tmpOffset    = nOffset + (nOffset < 0 ? this._length : 0),
			  tmpStart     = realStart + tmpOffset,
			  tmpEnd       = tmpStart + nLength;
		const wantStart    = nLength >= 0 ? tmpStart : tmpEnd,
			  wantEnd      = nLength >= 0 ? tmpEnd   : tmpStart,

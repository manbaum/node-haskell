...
const FArrayT = class {
	...
	*[Symbol.iterator](allowInfinity = false) {
	...
	slice(offset = 0, length = Infinity) {
		...
		const tmpStart     = this._start + nOffset + (nOffset < 0 ? this._length : 0),
			  tmpEnd       = tmpStart + nLength;
		const thisLimit    = limit(0, this._length),
			  newStart     = thisLimit(tmpStart),
			  newEnd       = thisLimit(tmpEnd),
			  newLength    = Math.abs(newEnd - newStart),
			  newReversed  = tmpEnd >= tmpStart ? this._reversed : !this._reversed;

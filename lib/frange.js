...
const FrangeT = class {
	...
	*[Symbol.iterator]() {
	...
	move(start, length, reversed) {
		...
		const newLength    = check(Math.trunc(length), [
			[ isNaN,		`* FrangeT#move: length.need.number`				],
			[ x => x < 0,	`* FrangeT#move: length.need.positive.number`		]]);
	...
	slice(offset = 0, length = Infinity) {
		...
		const newStart     = this._start + nOffset + (nOffset < 0 ? this._length : 0),
			  ...
			  rewReversed  = nLength >= 0 ? this._reversed : !this._reversed;
		return this.copy({ start: newStart, length: newLength, reversed: newReversed });
	}
	reverse() {
		check(this._length, [
			[ isInfinity,	`* FrangeT#reverse: cant.reverse.infinity.list`		]]);
...
const fmove    = fr => i => len => fr.move(i, Math.abs(len), len >= 0);
